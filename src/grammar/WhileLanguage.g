...
axiom_statement returns [ LinkedList<Axiom> result ]
    @init {$result = new LinkedList<Axiom>();}
        ...
        for (Expression e : $e1.divisors) {
            $result.add(new Axiom(new Position($start.getLine(), $start.getCharPositionInLine()),
                new LogicalExpression(new Position($start.getLine(), $start.getCharPositionInLine()), e, new NumericLiteral(
                    new Position($start.getLine(), $start.getCharPositionInLine()), "0"), new NotEqual())
                ));
        }
...
assert_statement returns [ Assertion ast, LinkedList<Expression> divisors ]
        : 'assert' e1=quantified_expression ';' {if (!error) {$ast = new Assertion(new Position($start.getLine(), $start.getCharPositionInLine()), $e1.ast);
...
assignment returns [ Assignment ast, LinkedList<Expression> divisors ]
    @init {$divisors = new LinkedList<Expression>();}
        ...
        | IDENT {LinkedList<Expression> idx = new LinkedList<Expression>();}
            ...
            {if (!error) {$ast = new ArrayAssignment(new Position($start.getLine(), $start.getCharPositionInLine()), $e2.ast, new Identifier($IDENT.text),
...
return_statement returns [ ReturnStatement ast, LinkedList<Expression> divisors ]
	: 'return' expression ';' {if (!error) {
        $ast = new ReturnStatement(new Position($start.getLine(), $start.getCharPositionInLine()), $expression.ast); $divisors = $expression.divisors;}}
	;
...
literal_expression returns [ Expression ast, LinkedList<Expression> divisors ]
        : INT_LITERAL {if (!error) {$ast = new NumericLiteral(new Position($start.getLine(), $start.getCharPositionInLine()),
            $INT_LITERAL.text); $divisors = new LinkedList<Expression>();}}
