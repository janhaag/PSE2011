...
model	returns [String example]	
	:	'(model'
		('(define-fun' id = IDENT  '('(IDENT  TYPE)*')' TYPE val = value ')' {$example += $id.text + "=" + $val.content + "\n";} )*
		...
		('(define-fun' id3 = (IDENT '!' INT) '('('('IDENT '!' INT TYPE')')+')' TYPE  '('ass = ite {$example += m.get($id3.text) + $ass.assignment;}'))')*
		')' 
	;

ite	returns[String assignment]
	:	'(=' IDENT '!' INT i = INT')'  val = value {$assignment = "[" + $i.text + "]" + "=" + $val.content + "\n"}(value | '('as=ite')'{$assignment += $as.assignment;})
	|	'(and'('(=' IDENT '!' INT i = INT')'{$assignment += "["+$i.text+"]";})+')' val=value {$assignment += "=" + $val.content + "\n";}(value | '('as=ite')'{$assignment += as;})		
	;
